...
node_t* Delete( node_t* root, int value ){
    ...
    if ( LeftOf( value, root ) )
        ...
        if ( root->left == NULL )
        {
            ...
            free( root );
            return newRoot;// trả về cho root cha con phải
        }
        if ( root->right == NULL )
        {
            node_t* newRoot = root->left;
            free( root );
            return newRoot;// trả về cho root cha con trái
        }
        // nếu root có cả 2 nút lá thì tìm nút trái nhất để thay thế & xóa nút trái nhất đó đi
        root->data = LeftMostValue( root->right );
        root->right = Delete( root->right, root->data );
    }
    return root;// trả về cho root cha
}
//-----------
